...
LINKFLAGS += $(shell grsi-config --HILData-libs)
...
all: include/HILDataVersion.h $(LIBRARY_OUTPUT) lib/libHILData.so $(HISTOGRAM_SO) $(EXECUTABLES)
...
lib/lib%.so: $(LIBRARY_OUTPUT) .build/histos/%.o | include/HILDataVersion.h lib
...
lib/lib%.so: $$(call lib_o_files,%) $$(call lib_dictionary,%) | include/HILDataVersion.h lib
	$(call run_and_test,$(CPP) -fPIC $^ $(SHAREDSWITCH)lib$*.so $(ROOT_LIBFLAGS) $(GRSI_LIBFLAGS) -o $@,$@,$(BLD_COLOR),$(BLD_STRING),$(OBJ_COLOR) )

lib/libHILData.so: $(LIBRARY_OUTPUT) $(MAIN_O_FILES) | include/HILDataVersion.h
