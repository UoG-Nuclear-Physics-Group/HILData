...
std::string TRlmdFile::Status(bool)
{
   return Form(HIDE_CURSOR " Processed event, have processed %.2fMB/%.2f MB              " SHOW_CURSOR "\r",
               (fInputStream.tellg() / 1000000.0), (FileSize() / 1000000.0));
}
...
bool TRlmdFile::Open(const char* filename)
{
   ...
   try {
      ...
      //debugFile<<"Total file size is "<<FileSize()<<" bytes"<<std::endl;
...
{
   fInputStream.close();
   BytesRead(FileSize());   // just to make sure we don't try to read more data after the file was closed
}
   ...
   if(BytesRead() < FileSize()) {
      ...
      case 2:   // normal event so we read the buffer data into the event and send it on
         ...
         try {
            ...
            ResizeBuffer(2 * fBufferHeader.dataLength);   // 2* accounts for the size of uint16_t
            fInputStream.read(BufferData(), 2 * fBufferHeader.dataLength);
            LastReadSize += static_cast<size_t>(fInputStream.gcount());
            IncrementBytesRead(LastReadSize);
            //std::cout<<"Read normal buffer with "<<2*fBufferHeader.dataLength<<"/"<<LastReadSize<<" bytes, now at byte "<<BytesRead()<<std::endl;
...
int TRlmdFile::GetRunNumber()
{
   ...
   std::size_t found = Filename().rfind(".rlmd");
